...
image = load_image(filename='sc1.jpg')
plot_image(image)
layer_tensor = model.layer_tensors[2]
layer_tensor                                                                     ## for small size image
img_result = optimize_image(layer_tensor, image,
                   num_iterations=10, step_size=6.0, tile_size=400,
                   show_gradient=True)

save_image(img_result, filename='deepdream_dc.jpg')

img_result = recursive_optimize(layer_tensor=layer_tensor, image=image,
                 num_iterations=10, step_size=3.0, rescale_factor=0.7,          # for large size image
...
for i in range (1,12):
    ...
    img_result = recursive_optimize(layer_tensor=layer_tensor, image=image,          # only maximizing 6th layer feauters
                 ...
                 num_repeats=4, blend=0.2)
    save_image(img_result, filename='deepdream_sc'+str(i)+'.jpg')

layer_tensor = model.layer_tensors[7][:,:,:,0:3]
img_result = recursive_optimize(layer_tensor=layer_tensor, image=image,
                 num_iterations=10, step_size=3.0, rescale_factor=0.7,
                 num_repeats=4, blend=0.2)
save_image(img_result, filename='deepdream_dc3.jpg')
